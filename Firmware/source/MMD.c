...
void MMD_refreshDisplay(void)
{
	...
	for( i = 0 ; i < MMD_MAX_SEGMENTS ; i++)
	{
		...
		for( j = 0 ; j < mmdSegment[i].length ; j++)
		{
			...
			MMD_DIGIT_PORT = addr;
			MMD_DATA_PORT = ~dataByte;
			MMD_DISPLAY_CONTROL = DISPLAY_ENABLE;	//enable the display					
			Delay10us(1);
			MMD_DISPLAY_CONTROL = DISPLAY_DISABLE;	//disable the display					
...
void WriteDataToDisplay(UINT8 digit, UINT8 data)
{
	...
	MMD_DISPLAY_CONTROL = DISPLAY_DISABLE;	//disable the display
	MMD_DIGIT_PORT = digit;					//set the address
	MMD_DATA_PORT = data;					// write data
	MMD_DISPLAY_CONTROL = DISPLAY_ENABLE;	//enable the display
	Delay10us(10);						//delay for the data to stabilize
	MMD_DISPLAY_CONTROL = DISPLAY_DISABLE;	//disable it again
